...
final class Helpers
{
    ...
    public static function appBasePath(array $config): string
    {
        ...
        if (preg_match('#/index\.php$#i', $scriptName) === 1) {
            $scriptDirectory = '/' . trim(substr($scriptName, 0, -strlen('/index.php')), '/');
            return $scriptDirectory === '/' ? '' : $scriptDirectory;
        }

        $documentRoot = realpath((string) ($_SERVER['DOCUMENT_ROOT'] ?? ''));
        $applicationRoot = realpath((string) ($config['app']['base_path'] ?? ''));
        if (
            $documentRoot !== false
            && $applicationRoot !== false
            && $applicationRoot !== $documentRoot
            && str_starts_with($applicationRoot, $documentRoot . DIRECTORY_SEPARATOR)
        ) {
            return '/' . str_replace(DIRECTORY_SEPARATOR, '/', substr($applicationRoot, strlen($documentRoot) + 1));
        }
    ...
    /** @param array<string, mixed> $config */
    public static function requestPath(array $config, ?string $uri = null): string
    {
        $requestPath = parse_url($uri ?? (string) ($_SERVER['REQUEST_URI'] ?? '/'), PHP_URL_PATH);
        $requestPath = is_string($requestPath) ? rawurldecode($requestPath) : '/';
        $basePath = self::appBasePath($config);
        ...
        if (str_starts_with($requestPath, '/index.php/')) {
            return substr($requestPath, strlen('/index.php'));
        }

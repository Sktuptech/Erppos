...
if ($action === 'login' && $method === 'POST') {
    ...
    Response::success([
        ...
        'redirect' => Helpers::appPath($config, '/dashboard'),
...
if ($action === 'change-password' && $method === 'POST') {
    ...
    $data = $validator->validated();
    $passwordRow = $database->fetchOne(
        'SELECT password_hash FROM users WHERE id = :id AND company_id = :company_id AND deleted_at IS NULL',
        ['id' => (int) $user['id'], 'company_id' => (int) $user['company_id']]
    );
    if ($passwordRow === null || !password_verify((string) $data['current_password'], (string) $passwordRow['password_hash'])) {
    ...
    $newHash = password_hash(
        ...
        (array) $config['security']['password_options']
    );
    $database->transaction(function ($db) use ($user, $newHash, $auditLogger): void {
        $db->execute(
            'UPDATE users

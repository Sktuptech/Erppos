...
if (PHP_SAPI === 'cli-server') {
    $developmentPath = Helpers::requestPath($config);
    if (
...
$requestPath = Helpers::requestPath($config);
...
if ($requestPath === '/') {
    Helpers::safeRedirect(Helpers::appPath($config, $auth->check() ? '/dashboard' : '/login'));
}

if ($requestPath === '/login' && $auth->check()) {
...
$pageRoutes = [
    ...
    '/settings' => ['views/pages/settings.php', 'settings.manage'],
